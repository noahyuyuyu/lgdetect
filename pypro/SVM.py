...
class GenData(object):
    def liner_data(self):
        mean1 = np.array([0, 2])  # 维度
        mean2 = np.array([2, 0])
        cov = [[0.8,0.6],[0.6,0.8]]  # 协方差矩阵
        X1 = np.random.multivariate_normal(mean1,cov,100)  # 生成一个多元正态分布矩阵
        y1 = np.ones(100)
...
class SVM(object):
    ...
    def kernelGen(self,x,y,kernel_type = 'liner',p=3):
        if kernel_type == 'liner':
            return np.dot(x,y)
        elif kernel_type == 'polynomial':
            return (1 + np.dot(x, y)) ** p  # 点积
        else :
    ...
    def fit(self,X,y,kernel_type = 'liner',p=3,C = None):
        ...
        P = cvxopt.matrix(np.outer(y,y)*K)  # 把array按照dims重新排成矩阵
